...
## Analiza wariancji - Czy istnieje sklep którego średnia sprzedaż różni się 
istotnie statystycznie od sprzedaży w innych sklepach?
...
dane %>%
    group_by(item) %>% 
    split(list(dane$item)) -> dane_podzielone_p
```
...
```{r}
lapply(dane_podzielone_p, function(x) analiza_produktu(x))
```
